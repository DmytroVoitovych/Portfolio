...
export const App = () => {
  ...
  const [showModal, setShowModal] = useState(
    () => JSON.parse(window.localStorage.getItem('stateModal')) ?? false
  );
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      showModal &&
        window.localStorage.setItem('stateModal', JSON.stringify(showModal));
      clearTimeout(id);
    }, 450);
  ...
  return (
    <>
      <div
        style={{
          ...controlStyle,
          position: showModal || (inView && isMobile) ? 'fixed' : '',
          overflowY: showModal ? 'scroll' : 'auto',
          top: showModal ? `-${location}px` : ``,
        }}
      >
        <Header
          onOpen={setShowModal}
          setScroll={setLocation}
          loc={{ refAbout, refProj, ref }}
          media={{ burger, isMobile }}
        />
        <main>
          <Hero about={setAbout} media={{ burger, isMobile, tablet, all }} />
          <Project
            proj={setProj}
            media={{ burger, isMobile, tablet, all, desctop }}
          />
          <Write
            onClose={funcToglle}
            toggle={showModal}
        ...
        {showModal && (
          <ContactMe
            onClose={funcToglle}
            toggle={showModal}
            media={{ burger, isMobile, tablet, extra }}
          />
        )}
        {<BtnUandD />}
      </div>
